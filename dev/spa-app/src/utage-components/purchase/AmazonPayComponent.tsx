...
const amazonPayPic = '/banner/amazonpay.webp';
...
interface AmazonPayComponentProps {
  email?: string;
  userName: string;
  userId: string;
  point: number;
  ...
  onPaidySuccess?: () => void;
  amazonSignature?: string;
  payload?: any;
}

const AmazonPayComponent: React.FC<AmazonPayComponentProps> = (props) => {
  ...
  const handleNextClick = () => {
    if (selectedCourse) {
      // ここで選択したコースの金額をクエリパラメータとして渡してリダイレクト
      router.push(`/amazon-pay-confirm?sid=${selectedCourse.money}`);
    }
  };
    ...
    <>
      ...
      <center>
        <div className={styles.topbanner}>
          <Image src={amazonPayPic} alt="amazonPay Banner" />
        </div>
      </center>
